...
import {
  Card, Typography, CardActionArea, CardContent, CardHeader, CardMedia, AppBar, Toolbar, IconButton, Button,
...
export default function Period1(props) {
  const [sectionOneText, setSectionOneText] = React.useState('');
  const [value, setValue] = React.useState('');
  const [error, setError] = React.useState(false);
  const [helperText, setHelperText] = React.useState('Choose one');

  React.useEffect(() => {
    fetch('/api/Content/period1')
      .then((response) => response.json())
      .then((response) => setSectionOneText(response.sectionOneText))
  })
